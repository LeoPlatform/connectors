...
module.exports = function (connection, fieldsTable) {
	...
	function del(event, callback) {
		...
		getFields(connection, event).then((table) => {
			...
			let delQuery =
				(fullDeletes ? `DELETE FROM ${tableName}` : `UPDATE ${tableName} SET ${deletedColumn} = ${deletedValue} ${auditdate}`) +
				` WHERE ${settings.id_column} ${whereSql}`;
