...
// RPL-6780. `combine()` groups records by natural key, so a delete keyed by a parent FK
...
// is covered end-to-end in rstreams-connector-datalake's test suite. What is testable
// here is the fold's handling of the sequence field, which is where it can be lost.
const SEQ = "__leo_seq__";

describe("combineRecords sequence carry-over (RPL-6780)", () => {
	it("a delete winning the fold takes the delete's sequence, not the earlier write's", () => {
		...
		expect(out.qty).to.equal(1); // RPL-5795: data still survives the close
		expect(out[SEQ]).to.equal(20);
	});

	it("a write winning the fold takes the write's sequence (later write reactivates, ES-2516)", () => {
